...
def run_agent(user_message: str, history: list) -> str:
    ...
    # 1. Build the messages list: system prompt + replayed history + new user message.
    ...
    try:
        # 2. Tool-calling loop, bounded by MAX_TOOL_ROUNDS so it can never spin forever.
        for _ in range(MAX_TOOL_ROUNDS):
            ...
            # Append the assistant message BEFORE the tool results (API ordering).
            messages.append(assistant_message)

            # Execute each requested tool and append its result, linked by tool_call_id.
            for tool_call in assistant_message.tool_calls:
                tool_name = tool_call.function.name
                # Arguments arrive as a JSON string. The LLM sometimes sends "" or
                # "null" for a no-argument tool (e.g. get_seasonal_conditions with no
                # season) — both must normalize to {} so dispatch_tool gets a dict.
        ...
        # Exit (b): round budget exhausted while still requesting tools. Force a final
        # text answer with tools disabled so the user still gets a response.
        ...
        # Malformed tool args, None content, or an API/network error — never crash the UI.
