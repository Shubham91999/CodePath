...
def lookup_plant(plant_name: str) -> dict:
    ...
    """
    # Normalize once: strip surrounding whitespace and lowercase for case-insensitive
    # comparison. Every comparison below is done against lowercased values.
    ...
    # 2. Display name match, then 3. alias match. Aliases are lowercased at
    # compare-time so matching is case-insensitive regardless of how they're stored.
    ...
    # Not found. The message is read by the LLM, not a human — tell it what happened,
    # that the database is limited, and how to behave (don't invent care details).
    return {
        "found": False,
        "name": normalized,
        "message": (
            f"No plant matching '{normalized}' was found in the database. The database "
            "covers 15 common houseplants. Ask the user to confirm the plant's common "
            "or scientific name, and do not invent specific care details that aren't "
            "grounded in a lookup result."
        ),
    }
