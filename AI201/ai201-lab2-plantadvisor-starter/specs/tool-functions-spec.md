...
```
The input is normalized once (strip + lowercase). For each plant, lowercase its
aliases at compare-time and test membership:

    normalized in [alias.lower() for alias in plant["aliases"]]

This is case-insensitive and matches any alias regardless of how it was stored.

At scale (thousands of plants) the per-plant scan becomes O(n*m). The faster
structure is a prebuilt inverted index — a single dict mapping every lowercased
name variant (key, display_name, each alias) to its plant slug — turning every
lookup into one O(1) dict access. For this 15-plant database the linear scan is
...
```
"No plant matching '{name}' was found in the database. The database covers 15 common
houseplants. Ask the user to confirm the plant's common or scientific name, and do not
invent specific care details that aren't grounded in a lookup result."

Rationale: the LLM is the reader, not a human log viewer. The message tells the agent
(1) what happened, (2) that the data source is limited in scope, and (3) how to behave
next — confirm the name and avoid hallucinating ungrounded care advice. That last clause
is what enables graceful degradation (Milestone 3).
```
...
```
yes — matches via the alias list, returns the Pothos entry.
```

**Test: does `"SNAKE PLANT"` return the snake plant entry?**
```
yes — uppercase input is lowercased by normalization and matches the display name.
```

**One edge case you discovered while implementing:**
```
Scientific names (e.g. "Epipremnum aureum") are NOT matched — the search order only
covers key, display_name, and aliases, not scientific_name. The not-found message
handles this gracefully by asking the user to confirm the common or scientific name
rather than failing silently. (Whitespace-padded input like "  monstera  " works
because we strip before comparing.)
```
...
```
yes — returns Winter data with detected_season = False, even though the current month
(June) maps to summer. An explicit valid season overrides auto-detection.
```
